...
public class TabbedGraphAreaController {

    private JTabbedPane graphAreaTabbedPane;
    private Map<String, JPanel> tabNames = new HashMap<String, JPanel>();
    
    public TabbedGraphAreaController(Application application) {
        this.graphAreaTabbedPane = application.getApplicationFrame().getTabbedPane();
    }
    
    public void showGraph(String title, Graph<JavaItem, JavaDependency>  graphInfo) {
        GraphPanelController graphController = new GraphPanelController(graphInfo);
    ...
    public Graph<JavaItem, JavaDependency> getActiveGraph() {
    	GraphPanel dependencyGraphPanel = (GraphPanel)graphAreaTabbedPane.getSelectedComponent();
